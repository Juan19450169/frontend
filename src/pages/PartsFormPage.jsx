...
function PartsFormPage() {
  ...
  const { parts, createParts, getPart, updateParts } = useParts();
  ...
  useEffect( ()=>{
    async function loadParts() {
      if(parms.id){
        const parts = await getPart(parms.id);
        setValue('name',parts.name);
        setValue('price',parts.price);
        setValue('year', parts.year);
        setValue('amount', parts.amount);
        setSelectedImage(`${server}/${parts.image}`);
  ...
  const onSubmit = handleSubmit(async (data) => { 
      ...
      if (parms.id) {
          await updateParts(parms.id, formData); // Usa FormData para actualización
      } else {
          await createParts(formData); // Usa FormData para creación
      }
  ...
  return (
    ...
    <div className="bg-blue-600 max-w-md w-full p-10 rounded-md">
      ...
      <h1 className="text-2x1 text-center font-bold my-4">Registro De Refacciones  </h1>
      <label htmlFor="name">Nombre:  </label>
        ...
        <input
          ...
          step="0.10"
          className="w-full bg-zinc-700 text-white px-4 py-2 rounded-md my-2"
          placeholder="Precio de la Refaccion"
          {...register("price", {
            valueAsNumber: true,
          })}
        />
        ...
        <input
          ...
          step="1"
          className="w-full bg-zinc-700 text-white px-4 py-2 rounded-md my-2"
          placeholder="Año de la Refaccion"
          {...register("year", {
            valueAsNumber: true,
          })}
        />
        ...
        <input
          ...
          step="0.10"
          className="w-full bg-zinc-700 text-white px-4 py-2 rounded-md my-2"
          placeholder="Cantidad de Refacciones"
          {...register("amount", {
            valueAsNumber: true,
          })}
        />
